...
function setupEventListeners() {
  ...
  document
    .getElementById("filterHang")
    .addEventListener("change", applyFilters);
  document
    .getElementById("filterLoai")
    .addEventListener("change", applyFilters);
...
function renderTable() {
  ...
  if (danhSachHienThi.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="10" class="text-center py-4">Không tìm thấy xe máy nào phù hợp.</td></tr>';
...
function editXe(id) {
  ...
  document.getElementById("modalXeMayLabel").innerText =
  ...
  document.getElementById("previewImg").src =
    xe.hinhAnh || "https://via.placeholder.com/400x300?text=Preview";
...
function deleteXe(id) {
  if (
    confirm(
      `Bạn có chắc chắn muốn xóa xe mã ${id} không? Hành động này không thể hoàn tác!`,
    )
  ) {
    // Xóa trong mảng gốc
...
function saveXeMayData() {
  ...
  if (action === "add") {
    ...
    xeData.thongSo = {
      congSuat: "Đang cập nhật",
      trongLuong: "Đang cập nhật",
      chieuDai: "Đang cập nhật",
      chieuRong: "Đang cập nhật",
      chieuCao: "Đang cập nhật",
      dungTichBinh: "Đang cập nhật",
      mucTieuHao: "Đang cập nhật",
    };
    xeData.danhGia = [];
